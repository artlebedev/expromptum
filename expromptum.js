	...
	xP.controls.register({name: 'foldable', base: 'fields', prototype: {
		...
		fold: function(fold, _duration){
			...
			if(fold){
				...
				this.$container.slideUp(_duration || this.duration);
			}else{
				this.$container.add(this.$label)
					.removeClass(this.folded_class)
					.addClass(this.unfolded_class);

				this.$container.slideDown(_duration || this.duration);
			}
				...
				this.change(function(){
					...
					xP.after(function(){
						...
						if(list.$element[0].options.length === 0){
							...
							var first_value = list.$element[0].options[0].value;
